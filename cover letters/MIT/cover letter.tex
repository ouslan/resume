...
Additionally I am familiar with orchestrating varios data process workflow using GitHub actions. Another of my responsibilities working with the Planning Board was designing and 
implementing a system that once one of the staging branches was merged into main the GitHub action would automatically build the Docker image and run various tests on the container. 
Next another script on the production server would scan for changes in the compiled image and would automatically update the old docker image with the new version.
